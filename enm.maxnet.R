...
##################################
# From pending update to ENMeval, v2.0.5, June 2024
...
maxnet.predict <- function(mod, envs, other.settings, clamp = FALSE) {
  ...
  if(inherits(envs, "BasicRaster") == TRUE) {
    ...
    mxnet.p <- predict(mod, envs.pts, type = other.settings$pred.type, 
                       clamp = clamp,  other.settings$other.args)
    ...
    pred <- predict(mod, envs, type = other.settings$pred.type, na.rm = TRUE, 
                    clamp = clamp, other.settings$other.args) %>% as.numeric()
  }
  return(pred)
}
